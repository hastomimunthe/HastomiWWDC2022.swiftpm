...
class Stage5: UIViewController {
    ...
    var buttonA: UIButton = {
        ...
        button.setTitle("A. Line up & wait, crash the bell & ask AVSEC for help", for: .normal)
    ...
    var buttonB: UIButton = {
        let button = UIButton()
        button.backgroundColor = .white
        button.setTitle("B. Ask the aircraft to return to main apron", for: .normal)
        button.titleLabel?.font = UIFont.systemFont(ofSize: 25)
        button.setTitleColor(UIColor.black, for: .normal)
    ...
    var buttonC: UIButton = {
        let button = UIButton()
        button.backgroundColor = .white
        button.setTitle("C. Ask the aircraft to take-off", for: .normal)
        button.titleLabel?.font = UIFont.systemFont(ofSize: 25)
        button.setTitleColor(UIColor.black, for: .normal)
        button.contentMode = .scaleAspectFit
        button.translatesAutoresizingMaskIntoConstraints = false
    ...
    var main2ImageView: UIImageView = {
        ...
        mainView.image = UIImage(named: "Situation5")
    ...
    @objc func actionRight(_ sender: UIButton){
        let alert = UIAlertController(title: " Correct ✅ Good Job!! ", message:  """
                ...
                maneuvering area and obstractionon that area .
            
        """, preferredStyle: .alert)
        alert.addAction(UIAlertAction(title: "Next Question", style: .default, handler: {(action) in
            self.navigationController?.pushViewController(Stage6(), animated: true)
        }) )
